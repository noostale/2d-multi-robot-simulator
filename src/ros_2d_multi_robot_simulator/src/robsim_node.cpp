 ...
 * 13. [ ] Implement a roslaunch alternative using a bash script
 */
...
/**
 * TOADD:
 * 1. Add a way to control the robots individually using the keyboard (Select the robot using the number keys and then control it using the arrow keys)
*/
...
int main(int argc, char **argv)
{
    ...
    for (const RobotConfig& robot_config : robotsConfig) {
        ...
        robot->radius = 1; // Set the radius of the robot
    ...
    while (ros::ok())
    {
        ...
        for (size_t i = 0; i < robots.size(); ++i) {
            ...
            pose_msg.header.frame_id = "robot_" + std::to_string(i); // Unique frame_id for each robot
            ...
            transform_stamped.child_frame_id = "robot_" + std::to_string(i);
            ...
            laser_scan_msg.header.frame_id = "robot_" + std::to_string(i); // We set the frame of the laser solidal to be the robot_pose frame
            ...
            cmd_vel_msg.header.frame_id = "robot_" + std::to_string(i);
            ...
            case 81: // left
                robots[i]->rv = 0.5;
                cmd_vel_msg.twist.angular.z = robots[i]->rv;
                break;
            case 82: // up
                robots[i]->tv = 0.5;
                cmd_vel_msg.twist.linear.x = robots[i]->tv;
                break;
            case 83: // right
                robots[i]->rv = -0.5;
                cmd_vel_msg.twist.angular.z = robots[i]->rv;
